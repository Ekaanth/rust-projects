...
async fn delete_user(firebase_client: &Firebase, id: &String) {
    let firebase = firebase_client.at("user").at(&id);
...
async fn main() {
    ...
    let firebase = Firebase::new("https://simple-crud-rust-default-rtdb.firebaseio.com/").unwrap();
    ...
    user.email = "updated.email@gmail.com".to_string();
